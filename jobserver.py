import Pyro4, daemon, subprocess, os, sys
from job import Job
...
class JobServer:
	...
	def _download(self):
		...
		dlProcess = subprocess.Popen(["youtube-dl", "-v", "-o", 
						self.currentJob.outFilename, 
						self.currentJob.video_url], 
						cwd=self.currentDir)
		return dlProcess.wait()
	
	def _play(self):
		self.isPlaying = 1
		playerProcess = subprocess.Popen(["omxplayer", self.currentJob.outFilename], cwd=self.currentDir)
		playerProcess.wait()
		self.isPlaying = 0
